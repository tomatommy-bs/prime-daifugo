...
describe('GameParty', () => {
  describe('constructor', () => {
    it('正しい初期状態でインスタンスが生成されること', () => {
      ...
      expect(state.players['0'].hand).toHaveLength(8)
      expect(state.players['0'].drawFlag).toBe(true)
      expect(state.players['1'].hand).toHaveLength(8)
      expect(state.players['1'].drawFlag).toBe(true)
      expect(state.field).toHaveLength(0)
  ...
  describe('move.draw', () => {
    it('現在のプレイヤーのみがmoveを実行できること', () => {
      ...
      // 現在のプレイヤー(0)がドローできる
      expect(party.getState().players['0'].drawFlag).toBe(true)
      party.move.draw('0', 1)
      ...
      expect(party.getState().players['0'].drawFlag).toBe(false)
    })
  })
})
